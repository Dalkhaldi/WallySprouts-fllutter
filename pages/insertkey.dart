...
class KeyPage extends StatefulWidget {
   int keyss;
   String mail;
   KeyPage(this.keyss,this.mail);

  @override
  _KeyPageState createState() => _KeyPageState(keyss,mail);
}
class _KeyPageState extends State<KeyPage> {

  int keyss;
  String mail;
  _KeyPageState(this.keyss,this.mail);

    final keysend = TextEditingController();
...
sendMail() async {
    ...
    final message = Message()
      ...
      ..subject =
          'Your verification code is : ${keysent}' //subject of the email
      //..text =
       
      ..html =
          "<h3>Thank you for joining Wally’s world ! :) "
          "your verification code is :${keysent}</h3>\n<p></p>"
...
chekey() async {
  var strkey1= keyss.toString();
 var strkey2= keysent.toString();
    var formdata = formState.currentState;
    if (formdata.validate()) {
      if(!resend)
     {
       if(strkey1==keysend.text)
      Navigator.of(context).push(MaterialPageRoute(
     builder:(context)=>PasswordPage (mail)));}
      if(resend)
     {
       if(strkey2==keysend.text)
      Navigator.of(context).push(MaterialPageRoute(
     builder:(context)=>PasswordPage (mail)));}
    else{
showToast('The code is incorrect! Check the code or resend',gravity: Toast.BOTTOM,duration: 3);
                ...
                child: Column(
                    ...
                    children: <Widget>[
                      Container(
                        padding: EdgeInsets.only(
                   right: 0),
                     child:Text("Enter 4 deget key that we send it on your email: "'\n',
